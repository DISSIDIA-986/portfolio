...
- [ ] **Fix GH Pages thumbnail basePath for local images** — Next.js `<Image>` with `unoptimized: true` in static export mode doesn't prefix local src paths (e.g. `/images/projects/yipaiji.svg`) with the `basePath` (`/portfolio`). All local SVG thumbnails show as broken on `dissidia-986.github.io/portfolio/`. CDN-hosted images (absolute URLs) are unaffected. Vercel deployment works fine. Fix: either prefix local image paths in `projects.ts` with basePath conditionally, or use a custom loader for `<Image>` in static export mode. ~15 min with CC.
